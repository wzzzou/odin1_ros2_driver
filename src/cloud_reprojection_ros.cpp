...
void CloudReprojectionRosNode::loadParameters()
{
    ...
    // Load camera parameters from calib.yaml in the shared runtime dir
    std::string calib_file = odin_ros_driver::GetOdinCalibFile();
    RCLCPP_INFO(this->get_logger(), "========== [CALIB] READ <- %s ==========", calib_file.c_str());
...
int main(int argc, char** argv)
{
    ...
    std::string calib_file = odin_ros_driver::GetOdinCalibFile();
    RCLCPP_INFO(temp_node->get_logger(), "========== [CALIB] WAIT <- %s ==========", calib_file.c_str());
